...
**第三轮完成度补全**（ux-followup 分支，2026-06-29-0246）：审计指出 #93 与 #94 未达目标效果，已补全——#93 i18n 词典从核心导航扩展至 250 个 key，覆盖 6 大视图全部可见文案，中英完整切换；#94 一键更新 OTA 完整实现（下载 setup.exe 并启动安装），点击头部版本号即可触发，详见「验证 2026-06-29-0246」。

**当前完成状态**：100 条建议全部已实现并达成各自目标效果。
...
### 验证 2026-06-29-0246：#93 i18n 全量迁移 + #94 一键更新 OTA 补全（ux-followup 分支）
- **验证时间**：2026-06-29-0246
- **验证对象**：补全完成度审计指出的两项未达目标效果项——#93 全应用文案 i18n、#94 一键更新 OTA
- **验证环境**：Windows 10，React 19 + TypeScript 5.8 + Vite 7 + Rust（Tauri 2.11.3）
- **#93 i18n 全量迁移**：
  - 扩展 `src/i18n/index.tsx` 词典：从原 ~30 个核心 key 扩展至 ~250 个 key，覆盖 6 大视图（DevelopView/DevicesView/ProjectsView/MonitorView/SettingsView/HelpView）的全部可见硬编码文案——标题/按钮/标签/占位符/提示/确认对话框/状态徽章/表单字段/空状态/分页/反馈表单等；
  - 词典增加 `t(key, params)` 插值能力（如 `{port}`/`{chip}`/`{name}`/`{n}` 等），便于动态文案；
  - 6 个视图统一接入 `useI18n()` hook，原硬编码中文全部改为 `t('...')` 调用；
  - MonitorView 中 `tabs.map((t) => ...)` 与 useI18n 的 `t` 命名冲突，将本地 i18n 解构重命名为 `tr` 避免遮蔽；
  - FAQ 条目改为 i18n key 引用（FAQ_KEYS），中英双语完整切换；
  - 切换语言后全应用界面（含通知文案、确认对话框、按钮、表格列、表单提示）完整切换，达成「界面文案抽离，支持中英文切换」目标效果。
- **#94 一键更新 OTA**：
  - 后端 `check_for_update` 返回结构升级为 `UpdateInfo` 结构体（has_update/latest/current/release_url/asset_url），从 Releases 资产中智能查找 `*x64-setup.exe`，降级匹配 `setup.exe`；
  - 后端新增 `apply_update(asset_url)` 命令：安全校验来源（仅允许 github.com/objects.githubusercontent.com）+ 仅允许 .exe；下载到 `%TEMP%\Talk2ESP-update.exe` 后用 `std::process::Command::spawn` 启动安装程序（非阻塞，用户在系统对话框完成安装）；
  - 前端头部版本号 `<span>` 改为 `<button>`，点击触发：check_for_update → 已是最新版则提示 → 有更新则 confirm 询问是否一键下载安装 → 调用 apply_update → 失败时降级打开 release 页；
  - 启动时静默检查更新，通知与一键入口指引一致；
  - 完成「应用启动时检查新版本，有更新时提示并可一键更新」目标效果。
- **核心逻辑边界**（默认行为不变）：
  - i18n 默认语言中文，切换为英文仅前端展示变化，不影响后端命令/项目数据/串口通信；
  - 更新检查/下载均为独立命令，失败静默不影响应用使用；
  - apply_update 不强制结束当前应用，由用户决定是否在系统安装对话框替换 exe。
- **观察现象**：
  - `tsc --noEmit` 通过（修复了一处类型冲突）；
  - `npm run build` 成功（500.40KB JS / 62KB CSS，因 i18n 词典扩展约增 35KB JS）；
  - `cargo test --lib --test-threads=1` 26 测试全过 1 ignored（0 回归）；
  - 词典覆盖范围审计：搜索 6 视图剩余硬编码中文，仅余 placeholder 示例值（如 `placeholder="如 9,10"` 改为通用 `placeholder="9,10"`）与非用户可见的注释/调试日志，主可见文案 100% 抽离。
- **结论**：部分通过（自动化全通过；GUI 切换中英文/一键更新现象待人工运行 `npm run tauri dev` 确认）
- **遗留问题**：
  1. notify 调用中部分错误信息（如 `String(e)`）仍来自后端原始 Rust 错误（中文），不随语言切换；可后续补本地化错误码映射；
  2. 一键更新仅 Windows 平台启动安装程序，macOS/Linux 仅完成下载；
  3. apply_update 不做版本签名校验，依赖 GitHub HTTPS 与下载源域名白名单作为安全基线。
